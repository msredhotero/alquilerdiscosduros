...
class Servicios {
	...
	function camposTablaView($cabeceras,$datos,$cantidad) {
		...
		switch ($cantidad) {
			...
			case 93:
				...
				$classMod = 'varmodificar';
				$classEditar = 'varver';
				$iconoEditar = 'glyphicon glyphicon-search';
				$lblEditar	  = 'Ver';
				$classEli = 'varborrar';
				$idresultados = "resultados";
				break;
	...
	function camposTabla($accion,$tabla,$lblcambio,$lblreemplazo,$refdescripcion,$refCampo) {
		...
		$label  = '';
		if ($tabla == 'dbordenes') {
			$ocultar = array("fechacreacion","fechamodi","usuacrea","usuamodi","tramado","roller","refventas");
		} else {
			$ocultar = array("fechacreacion","fechamodi","usuacrea","usuamodi","refpresupuesto");
		}
							...
							if (strpos($row[1],"bit") !== false) {
								...
								if (strpos($row[1],"date") !== false) {
									...
									if ($row[0] == "fechapago") {
										...
										<div class="form-group col-md-6" style="display:'.$lblOculta.'">
											<label for="'.$campo.'" class="control-label" style="text-align:left">'.$label.'</label>
											<div class="input-group date form_date col-md-6 '.$campo.'" data-date="" data-date-format="dd MM yyyy" data-link-field="'.$campo.'" data-link-format="yyyy-mm-dd">
...
<span class="glyphicon glyphicon-time"></span>
	...
	function camposTablaModificar($id,$lblid,$accion,$tabla,$lblcambio,$lblreemplazo,$refdescripcion,$refCampo) {
		...
		if ($tabla == 'dbordenes') {
			...
			$ocultar = array("fechacreacion","fechamodi","usuacrea","usuamodi","refpresupuesto","fecha");
		}
